        ...
        this.description = {
            ...
            properties: [
                ...
                },
                {
                    ...
                    type: 'string',
                    default: '',
                    description: 'Base image URL — switches generation to image-to-image editing. Use Brand and Dimensions do not apply on this branch.',
                    displayOptions: { show: { resource: ['aiImage'], operation: ['generate'] } },
                },
                {
                    displayName: 'Model',
                ...
                },
                {
                    ...
                    type: 'boolean',
                    default: false,
                    description: 'Whether to apply the workspace brand knowledge (resolved server-side, no brand ID accepted). Text-to-image only — ignored when Image URL is set; brand_applied is always false on the image-to-image branch.',
                    displayOptions: { show: { resource: ['aiImage'], operation: ['generate'] } },
                },
                {
                ...
                },
                {
                    ...
                    description: 'Only used by Estimate. Generate infers the mode automatically from Image URL / Reference Image URLs.',
                ...
                },
                {
                    ...
                    type: 'string',
                    default: '',
                    description: 'Source image URL — switches generation to image-to-video. Mutually exclusive with Reference Image URLs.',
                    displayOptions: { show: { resource: ['aiVideo'], operation: ['generate'] } },
                },
                {
                    ...
                    type: 'string',
                    default: '',
                    description: 'Comma-separated image URLs (max 8) — switches generation to reference-to-video. Mutually exclusive with Image URL.',
                    displayOptions: { show: { resource: ['aiVideo'], operation: ['generate'] } },
                },
                {
                    displayName: 'Style',
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'aiImage' && operation === 'generate') {
                    ...
                    const dimensions = this.getNodeParameter('aiImageDimensions', i) || '';
                    const enhancePrompt = this.getNodeParameter('aiImageEnhancePrompt', i);
                    const body = { prompt };
                    if (imageUrl)
                ...
                if (resource === 'aiImage' && operation === 'runTool') {
                    const workspaceId = this.getNodeParameter('workspaceId', i);
                    const toolKey = this.getNodeParameter('aiImageToolKey', i);
                    const body = {};
                    if (toolKey === 'image-to-image') {
                        ...
                        if (model)
                            body.model = model;
                        const enhancePrompt = this.getNodeParameter('aiImageToolEnhancePrompt', i);
                        if (enhancePrompt)
                            body.enhance_prompt = enhancePrompt;
                    }
                    ...
                    if (toolKey === 'upscale' || toolKey === 'headshot' || toolKey === 'face-swap' || toolKey === 'product-image') {
                        const resolution = this.getNodeParameter('aiImageToolResolution', i) || '';
                        if (resolution)
                            body.resolution = resolution;
                    }
                    if (toolKey === 'headshot' || toolKey === 'product-image') {
                        const aspectRatio = this.getNodeParameter('aiImageToolAspectRatio', i) || '';
                        if (aspectRatio)
                    ...
                    if (toolKey === 'product-image') {
                        ...
                        const instructions = this.getNodeParameter('aiImageToolInstructions', i) || '';
                        if (instructions)
                ...
                if (resource === 'aiVideo' && operation === 'estimate') {
                    const workspaceId = this.getNodeParameter('workspaceId', i);
                    const durationSeconds = this.getNodeParameter('aiVideoDuration', i);
                    const model = this.getNodeParameter('aiVideoModel', i) || '';
                    const resolution = this.getNodeParameter('aiVideoResolution', i) || '';
                    const aspectRatio = this.getNodeParameter('aiVideoAspectRatio', i) || '';
                    const generationMode = this.getNodeParameter('aiVideoGenerationMode', i) || '';
                    const enableAudio = this.getNodeParameter('aiVideoEnableAudio', i);
                    const enhancePrompt = this.getNodeParameter('aiVideoEnhancePrompt', i);
                    const body = {};
                ...
                if (resource === 'aiVideo' && operation === 'generate') {
                    ...
                    const enableAudio = this.getNodeParameter('aiVideoEnableAudio', i);
                    const enhancePrompt = this.getNodeParameter('aiVideoEnhancePrompt', i);
                    const style = this.getNodeParameter('aiVideoStyle', i) || '';
                    const useBrand = this.getNodeParameter('aiVideoUseBrand', i);
                    const body = { prompt };
                ...
                if (resource === 'aiVideo' && operation === 'runTool') {
                    const workspaceId = this.getNodeParameter('workspaceId', i);
                    const toolKey = this.getNodeParameter('aiVideoToolKey', i);
                    const body = {};
                ...
                if (resource === 'aiVideo' && operation === 'list') {
                    ...
                    const status = this.getNodeParameter('aiVideoJobStatus', i) || '';
                    options.method = 'GET';
                ...
                if (resource === 'post' && (operation === 'create' || operation === 'update')) {
                    ...
                    const hasInstagramOptions = this.getNodeParameter('hasInstagramOptions', i, false);
                    if (hasInstagramOptions) {
                        ...
                        if (instagramTrialReelEnabled) {
                            const graduationStrategy = this.getNodeParameter('instagramTrialReelGraduationStrategy', i, 'SS_PERFORMANCE') || 'SS_PERFORMANCE';
                            options.body.instagram_options = options.body.instagram_options || {};
                            options.body.instagram_options.trial_reel = {
                                enabled: true,
                    ...
                    if (hasLinkedinOptions) {
                        ...
                        const linkedinEnablePoll = this.getNodeParameter('linkedinEnablePoll', i, false);
                        ...
                        if (Object.keys(linkedinOptions).length > 0) {
                            options.body.linkedin_options = linkedinOptions;
                        }
                    }
                    ...
                    if (useApprovalWorkflow) {
                        ...
                        const workflowAction = operation === 'update'
                            ? (this.getNodeParameter('approvalWorkflowAction', i, '') || '').trim()
                            : '';
                        ...
                        options.body.approval_workflow = approvalWorkflow;
                    }
